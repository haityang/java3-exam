...
public class AddresstableDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory.getLogger(AddresstableDAO.class);
	// property constants
	...
	public Addresstable findById(java.lang.Integer id) {
		...
		try {
			Addresstable instance = (Addresstable) getSession().get("com.dao.Addresstable", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Addresstable instance) {
		...
		try {
			List results = getSession().createCriteria("com.dao.Addresstable").add(Example.create(instance)).list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Addresstable instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "from Addresstable as model where model." + propertyName + "= ?";
	...
	public Addresstable merge(Addresstable detachedInstance) {
		...
		try {
			Addresstable result = (Addresstable) getSession().merge(detachedInstance);
